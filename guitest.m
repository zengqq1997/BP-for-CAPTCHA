...
% Last Modified by GUIDE v2.5 02-Jan-2019 16:03:06
...
show=get(handles.edit1,'string');
show=str2num(show);
epochs=str2num(get(handles.edit2,'string'));
goal=str2double(get(handles.edit3,'string'));
learnRate=str2double(get(handles.edit4,'string'));
main(show,epochs,goal,learnRate);
...
[handles.rightdata,rate]=testbp();

handles.data=handles.rightdata;
axes(handles.axes3);pie(handles.data, {'right', 'wrong'})
set(handles.edit5,'string',num2str(rate))
...
global str filename
imgname=filename;
mans=testcode(str,imgname);
set(handles.edit6,'string',num2str(mans));
...
global str filename
[filename,pathname]=uigetfile({'*.jpg'},'´ò¿ªÍ¼Æ¬');
...
% --- Executes on mouse press over axes background.
function piephoto_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to piephoto (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
